...
class MocapViewer(glut_viewer.Viewer):
    ...
    ```
    python fairmotion/viz/amass_visualizer.py \
        --body-model-file $SMPL_MODEL_NPZ_FILE \
        --amass-files $AMASS_NPZ_FILE1
    ```

    To visualize more than 1 motion sequence side by side, append more files 
    to the `--amass-files` argument. Set `--x-offset` to an appropriate float 
    value to add space separation between characters in the row.

    ```
    python fairmotion/viz/amass_visualizer.py \
        --body-model-file $SMPL_MODEL_NPZ_FILE \
        --amass-files $AMASS_NPZ_FILE1 $AMASS_NPZ_FILE2 $AMASS_NPZ_FILE3 \
        --x-offset 2
    ```
    ...
    def _render_pose(self, pose, body_model, color):
        ...
        for j in skel.joints:
            ...
            gl_render.render_point(pos, radius=0.015 * self.scale, color=color)
...
def main(args):
    ...
    if args.body_model_file:
        bm = amass.load_body_model(bm_path=args.body_model_file)

    if args.amass_files:
        motions = [
            amass.load(file=filename, bm=bm)
            for filename in args.amass_files
        ]

    for i in range(len(motions)):
        motion_ops.rotate(motions[i], conversions.Ax2R(conversions.deg2rad(-90)))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Visualize AMASS file with block body"
    )
    parser.add_argument("--amass-files", type=str, nargs="+", required=False)
    parser.add_argument("--body-model-file", type=str, required=False)
